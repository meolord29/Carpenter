...
| cmd | input | `data` (ok) |
|-----|-------|-------------|
| `register [--app opencode]` | --app (default `opencode`) | `{"app":"opencode","path":"~/.config/opencode/skills/carpenter/SKILL.md","version":"0.8.1","installed":true}` — writes `SKILL.md` + merges `"skill":{"carpenter":"allow"}`; `agents` ⇒ `ValidationError` |
| `register --app claude-code` | --app | `{"app":"claude-code","path":"~/.claude/skills/carpenter/SKILL.md","version":"0.8.1","installed":true}` — writes `SKILL.md` into `~/.claude/skills/` (auto-discovered — no permission merge) |
