...
| cmd | input | `data` (ok) |
|-----|-------|-------------|
| `register [--app opencode]` | --app (default `opencode`) | `{"app":"opencode","path":"~/.config/opencode/skills/carpenter/SKILL.md","version":"0.5.0","installed":true}` — writes `SKILL.md` + merges `"skill":{"carpenter":"allow"}`; `claude-code`/`agents` ⇒ `ValidationError` |
