...
`scripts/install.sh` uses). The release channel (adr/020) picks the base URL:
`stable` (default) follows the Latest release published from the `release`
branch; `--channel edge` follows the rolling prerelease published from
`pre-release`. Both modes refresh the skill of every **registered**
...
[adr/020](../adr/020-branch-governed-channels.md).
